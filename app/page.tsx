...
export default function Home(){
 ...
 return <main className="shell">
  ...
  {tab==='recipes'&&<section><div className="sectionTitle"><div><p className="kicker">RECIPE LIBRARY</p><h1>MY RECIPES.</h1></div><button className="primary" onClick={()=>setEditing(emptyRecipe())}><Plus/> ADD RECIPE</button></div><div className="search"><Search/><input placeholder="Search recipe, brewer, bean…" value={query} onChange={e=>setQuery(e.target.value)}/></div><div className="cards">{filtered.map(r=><article className="recipeCard" key={r.id}><div className="cardTop"><span className="tag">{r.method}</span><button className="iconBtn" onClick={()=>favorite(r)}>{r.favorite?'♥':'♡'}</button></div><h2>{r.name}</h2><p>{r.beanName||'Any beans'}</p><div className="specGrid"><span><b>{r.coffee}g</b> coffee</span><span><b>{r.water}g</b> liquid</span><span><b>1:{r.ratio}</b> base ratio</span><span><b>{r.temp}°</b> temp</span></div><div className="cardActions"><button onClick={()=>{setSelected(null);setBrewMode(null);setEditing(r)}}><Pencil size={17}/> EDIT</button><button onClick={()=>{setEditing(null);setBrewMode(null);setSelected(r);setScaleDose(r.coffee)}}>OPEN <ChevronRight size={17}/></button><button onClick={()=>{prepareBrew(r)}}><Play size={17}/> BREW</button></div></article>)}</div></section>}
  {tab==='beans'&&<Beans data={data} update={update}/>} {tab==='journal'&&<Journal data={data} update={update}/>} 
  {editing&&<RecipeEditor recipe={editing} beans={data.beans} onClose={()=>setEditing(null)} onSave={saveRecipe}/>} {selected&&<RecipeDetail recipe={selected} scaled={scaled(selected,scaleDose)} dose={scaleDose} setDose={setScaleDose} onClose={()=>setSelected(null)} onEdit={()=>{const r=selected;setSelected(null);setEditing(r)}} onDelete={()=>deleteRecipe(selected.id)} onBrew={()=>prepareBrew(scaled(selected,scaleDose))} onSaveVariant={()=>{const v={...scaled(selected,scaleDose),id:uid(),name:selected.name+' · '+scaleDose+'g version',parentId:selected.id,createdAt:new Date().toISOString().slice(0,10)};saveRecipe(v)}}/>}
  {brewPickRecipe&&<div className="modal"><div className="modalBox beanPickModal">
   <div className="modalHead"><div><p className="kicker">BEFORE YOU BREW</p><h2>CHOOSE TODAY'S BEANS.</h2><p>{brewPickRecipe.name}</p></div><button className="iconBtn" onClick={()=>setBrewPickRecipe(null)}><X/></button></div>
   <p className="note">Select the beans you're using for this brew. We'll use them to keep your Journal and bean stock up to date.</p>
   <Field label="BEANS FOR THIS BREW">
    <select value={brewBeanId} onChange={e=>setBrewBeanId(e.target.value)}>
     <option value="">Custom / unspecified beans</option>
     {data.beans.map(b=><option value={b.id} key={b.id}>{b.name}{b.roastery?` · ${b.roastery}`:''} · {b.origin}</option>)}
    </select>
   </Field>
   {brewBeanId&&(()=>{const b=data.beans.find(x=>x.id===brewBeanId);return b?<div className="selectedBeanPreview">
    <div className="selectedBeanTitle"><BeanIcon/><span><b>{b.name}</b><small>{b.roastery||'Roastery not set'} · {b.species||'Arabica'}</small></span></div>
    <div className="selectedBeanFacts"><span><b>{b.origin}</b><small>ORIGIN</small></span><span><b>{b.process}</b><small>PROCESS</small></span><span><b>{b.roast}</b><small>ROAST</small></span><span><b>{b.stock}g</b><small>STOCK</small></span></div>
   </div>:null})()}
   <div className="modalActions">
    <button className="secondary" onClick={()=>setBrewPickRecipe(null)}>CANCEL</button>
    <button className="primary" onClick={()=>{
      const b=data.beans.find(x=>x.id===brewBeanId);
      setBrewMode({...brewPickRecipe,beanId:b?.id,beanName:b?.name||'Custom / unspecified',origin:b?.origin||brewPickRecipe.origin,process:b?.process||brewPickRecipe.process,roast:b?.roast||brewPickRecipe.roast});
      setBrewPickRecipe(null);setBrewStart(null);setSeconds(0);
    }}><Play/> CONTINUE TO BREW</button>
   </div>
  </div></div>}
...
function RecipeEditor({recipe,beans,onClose,onSave}:{recipe:Recipe;beans:Bean[];onClose:()=>void;onSave:(r:Recipe)=>void}){
 ...
 return <div className="modal"><div className="modalBox wide">
  ...
  <div className="formGrid">
   ...
   <Field label="Default bean (optional)"><select value={r.beanName} onChange={e=>chooseBean(e.target.value)}><option value="">Any beans / choose when brewing</option>{beans.map(b=><option key={b.id}>{b.name}{b.roastery?` · ${b.roastery}`:''}</option>)}</select></Field>
